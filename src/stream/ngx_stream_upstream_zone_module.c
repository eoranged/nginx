...
static void ngx_stream_upstream_zone_copy_hc(
...
static ngx_int_t ngx_stream_upstream_zone_cmp_resolved(const void *one,
    const void *two);
static ngx_int_t ngx_stream_upstream_zone_cmp_sockaddr(struct sockaddr *one,
    socklen_t one_len, struct sockaddr *two, socklen_t two_len);
static ngx_int_t ngx_stream_upstream_zone_addr_match(
    ngx_stream_upstream_rr_peer_t *peer, ngx_stream_upstream_rr_peer_t *template,
    ngx_stream_upstream_host_t *host, ngx_resolver_addr_t *addr);
static void ngx_stream_upstream_zone_sort_peers(
    ngx_stream_upstream_rr_peers_t *peers, ngx_stream_upstream_host_t *host,
    ngx_stream_upstream_rr_peer_t *template, ngx_resolver_addr_t *addrs,
    ...
    ngx_stream_upstream_srv_conf_t *uscf, ngx_stream_upstream_srv_conf_t *ouscf)
{
    ...
    ngx_stream_upstream_zone_copy_hc(peers, opeers);
    ...
    ngx_stream_upstream_zone_copy_hc(backup, opeers ? opeers->next : NULL);
...
static void
ngx_stream_upstream_zone_copy_hc(ngx_stream_upstream_rr_peers_t *peers,
    ngx_stream_upstream_rr_peers_t *opeers)
{
    ngx_stream_upstream_rr_peer_t  *peer, *opeer;
    ...
    for (peer = peers->peer; peer; peer = peer->next) {
        ...
        for (opeer = opeers->peer; opeer; opeer = opeer->next) {
            ...
            peer->down &= ~NGX_STREAM_UPSTREAM_HC_DOWN;
            peer->down |= opeer->down & NGX_STREAM_UPSTREAM_HC_DOWN;
            break;
        }
    }
    ...
    if (ctx->naddrs > 1) {
        ...
        if (addrs == NULL) {
            ...
            ngx_sort(addrs, ctx->naddrs, sizeof(ngx_resolver_addr_t),
                     ngx_stream_upstream_zone_cmp_resolved);
        }
    }
...
again:
    ...
    ngx_stream_upstream_zone_sort_peers(peers, host, template, addrs,
    ...
    while (++i < ctx->naddrs) {
        ngx_stream_upstream_zone_unmark_addr(&addrs[i]);
    }
...
static ngx_int_t
ngx_stream_upstream_zone_cmp_resolved(const void *one, const void *two)
{
    ngx_int_t              rc;
    ...
    return ngx_stream_upstream_zone_cmp_sockaddr(first->sockaddr,
                                                 first->socklen,
                                                 second->sockaddr,
                                                 second->socklen);
}
...
static ngx_int_t
ngx_stream_upstream_zone_cmp_sockaddr(struct sockaddr *one, socklen_t one_len,
...
static ngx_int_t
ngx_stream_upstream_zone_addr_match(ngx_stream_upstream_rr_peer_t *peer,
    ngx_stream_upstream_rr_peer_t *template, ngx_stream_upstream_host_t *host,
...
static void
ngx_stream_upstream_zone_sort_peers(ngx_stream_upstream_rr_peers_t *peers,
    ngx_stream_upstream_host_t *host, ngx_stream_upstream_rr_peer_t *template,
    ngx_resolver_addr_t *addrs, ngx_uint_t naddrs, ngx_uint_t backup,
    u_short min_priority)
{
    ngx_uint_t                      i, addr_backup;
    ngx_resolver_addr_t            *addr;
    ngx_stream_upstream_rr_peer_t  *peer, *next, *head, *resolved;
    ngx_stream_upstream_rr_peer_t **last, **rlast, **peerp;
    ...
    for (i = 0; i < naddrs; i++) {
        ...
        for (peerp = &resolved; *peerp; peerp = &peer->next) {
            ...
            if (ngx_stream_upstream_zone_addr_match(peer, template, host, addr)
                != NGX_OK)
            {
                continue;
            }
